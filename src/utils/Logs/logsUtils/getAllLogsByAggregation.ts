...
import {createAggregationPipelineForLogs} from "../createAggregationPipelineForLogs";
...
const getAllLogsByAggregation = async ({filters, page, pageSize}: inputObjectGetDataByAggregation2) => {
    ...
    const myPipelineForTickets = createAggregationPipelineForLogs({matchConditions, page, pageSize});

    const result = await LogModel.aggregate(myPipelineForTickets);
...
export default getAllLogsByAggregation;
